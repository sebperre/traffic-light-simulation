...
def simulate_intersection(simulation_type, approaches, merged_arrival_times, free_flow_speed, current_speed, intersection_length, light_cycle_duration: float = 30, default_processing_times=None):
    ...
    for arrival_time, approach, lane in merged_arrival_times:
        ...
        # lane = 0 if lane in [0,1,2,3] else 1
        # lane = 0
...
def main():
    ...
    num_simulations = 100 # we simulate all datetimes num_simulations times
    ...
    for date in datetimes: # for each date
        throughput_diffs = []
        avg_waiting_time_diffs = []
        print(f"Date Simulated is {date}")
        for i in range(num_simulations): # run num_simulations 
            fixed_processing_time = None
            throughput_fixed=None
            throughput_dynamic=None
            waiting_time_fixed=None
            waiting_time_dynamic=None
            average_waiting_time_fixed=None
            average_waiting_time_dynamic=None
            ...
            for simulation_type in [FIXED, DYNAMIC]:
                waiting_times,average_waiting_time,events_per_second,max_queue_len,avg_intersection_clear_time,intersection_processed_times \
                    = run_simulation(simulation_type, date, OAKFOUL_VIC, intersection_coords, approaches, num_vehicles, intersection_length, fixed_processing_time)
                ...
                if simulation_type==FIXED:
                    fixed_processing_time = intersection_processed_times

                    throughput_fixed = events_per_second
                    waiting_time_fixed = waiting_times
                    average_waiting_time_fixed = average_waiting_time
                else:
                    throughput_dynamic = events_per_second
                    waiting_time_dynamic = waiting_times
                    average_waiting_time_dynamic = average_waiting_time
            
            throughput_diffs.append(throughput_fixed - throughput_dynamic)
            avg_waiting_time_diffs.append(average_waiting_time_fixed - average_waiting_time_dynamic)
        print("Confidence Interval for CRN of Throughputs:", confidence_interval(throughput_diffs, 0.05))
        print("Confidence Interval for CRN of Average Waiting Time:", confidence_interval(avg_waiting_time_diffs, 0.05))
        print("\n")
